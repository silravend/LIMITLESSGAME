...
contract Limitless {
    ...
    uint constant HOUSE_EDGE_MINIMUM_AMOUNT = 30000;

    // Bets lower than this amount do not participate in jackpot rolls (and are
    // not deducted JACKPOT_FEE).
    uint constant MIN_JACKPOT_BET = 10000000;

    // Chance to win jackpot (currently 0.1%) and fee deducted into jackpot fund.
    uint constant JACKPOT_MODULO = 1000;
    uint constant JACKPOT_FEE = 100000;

    // There is minimum and maximum bets.
    uint constant MIN_BET = 10000000;
    uint constant MAX_AMOUNT = 200000000000000;
    ...
    uint constant BET_EXPIRATION_BLOCKS = 2500;
    ...
    function placeBet(uint betMask, uint modulo, uint commitLastBlock, bytes32 commit, bytes32 r, bytes32 s) external payable {
    ...
    function settleBet(uint reveal, uint blockNumber, bytes32 blockHash) external onlyCroupier {
        ...
        require (placeBlockNumber == blockNumber);
    ...
    function verifyMerkleProof(bytes32 seedHash, uint offset) pure private returns (bytes32 blockHash, bytes32 uncleHash) {
